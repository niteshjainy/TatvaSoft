...
let authController = {
  ...
  changePassword(req, res, next) {
    User.find({
      ...
      .then((data) => {
        ...
        if (user) {
          bcrypt
            .compare(bodyData.current_password, user.password)
            .then((result) => {
              if (result) {
                bcrypt
                  .hash(bodyData.new_password, 10)
                  .then((hash) => {
                    let userData = {
                      password: hash,
                    };
    ...
    if (email) {
      User.find({ where: { email: email } })
        .then((user) => {
          if (user) {
            user
              .updateAttributes({ verify_token: token })
              .then((data) => {
                let options = {
                  to: user.email,
  ...
  checkEmailExistsDriverUpdate(req, res, next) {
    ...
    Driver.find({ where: { email, id: { $ne: req.user.id } } })
      .then((driver) => {
        if (driver) {
          res.status(400).json({
            success: false,
  ...
  checkEmailExistsCustomerUpdate(req, res, next) {
    let email = req.body.email;
    Customer.find({ where: { email, id: { $ne: req.user.id } } })
      .then((customer) => {
        if (customer) {
          res.status(400).json({
            success: false,
  ...
  checkDriverAuthenticated(req, res, next) {
    ...
    let response = {};
    if (token) {
      Driver.findOne({ where: { access_token: token } })
        .then((driver) => {
          if (!driver) {
            let error = new Error("Invalid access token");
            error.status = 401;
            ...
            if (driver.status === "inactive") {
              Driver.update(
                { access_token: null, device_id: null, device_type: null },
                { where: { id: driver.id } }
              )
              ...
              let error = new Error(
                "Your account has been deactivated. Please contact to support for any query."
              );
              error.status = 403;
              next(error);
            } else {
  ...
  checkCustomerAuthenticated(req, res, next) {
    ...
    let response = {};
    if (token) {
      Customer.findOne({ where: { access_token: token } })
        .then((customer) => {
          if (!customer) {
            let error = new Error("Invalid access token");
            error.status = 401;
  ...
  checkWebdeskAuthenticated(req, res, next) {
    ...
    if (token) {
      Customer.findOne({
        where: { access_token: token, customer_type: "webdesk" },
      })
        .then((customer) => {
          if (!customer) {
            let error = new Error("Invalid access token");
